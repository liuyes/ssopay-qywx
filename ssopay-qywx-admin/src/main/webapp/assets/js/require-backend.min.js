        ...
        api: {
            ...
            fixurl: function (url) {
                if (url.substr(0, 1) !== "/") {
                    ...
                    if (!r.test(url)) {
                    	url = ctxUrl + Config.moduleurl + "/" + url;
                    }
                }
                return url;
            },
...
define('backend',['fast', 'moment'], function (Fast, Moment) {
    var Backend = {
        ...
        init: function () {
            //公共代码
            //配置Toastr的参数
            Toastr.options.positionClass = Config.actionname == 'index' ? "toast-top-right-index" : "toast-top-right";
            ...
            if (obj.size() > 1) {
                if (/BETWEEN$/.test(sym)) {
                    ...
                    if (typeof vObjCol.process === 'function') {
                        ...
                        value_begin = Moment(value_begin);
                        value_end = Moment(value_end);
                        if ('00:00:00' === Hms) {
                            value_end += 86399000;//结束时间加上一天的毫秒数
                        }
                    }
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                $(toolbar).on('click', Table.config.addbtn, function () {
                    var ids = Table.api.selectedids(table);
                    Fast.api.open(options.extend.add_url, __('Add'));
                });
                // 批量编辑按钮事件
                $(toolbar).on('click', Table.config.editbtn, function () {
                    ...
                    $.each(ids, function (i, j) {
                        Fast.api.open(options.extend.edit_url + "/" + j, __('Edit'));
                    });
                });
                // 批量操作按钮事件
                ...
                $(table).on("click", "[data-id].btn-edit", function (e) {
                    e.preventDefault();
                    Fast.api.open(options.extend.edit_url + "/" + $(this).data("id"), __('Edit'));
                });
            ...
            events: {
                operate: {
                    'click .btn-editone': function (e, value, row, index) {
                        ...
                        Fast.api.open(options.extend.edit_url + "/" + row[options.pk], __('Edit'));
                    },
            ...
            formatter: {
                ...
                disable: function (value, row, index, custom) {
                	//颜色状态数组,可使用red/yellow/aqua/blue/navy/teal/olive/lime/fuchsia/purple/maroon
                    var colorArr = {"1": 'success', "-1": 'danger'};
                    var valueArr = {"1": '有效', "-1": '无效'};
                    //如果有自定义状态,可以按需传入
                    if (typeof custom !== 'undefined') {
                        colorArr = $.extend(colorArr, custom);
                    }
                    ...
                    value = value.charAt(0).toUpperCase() + value.slice(1);
                    //渲染状态
                    var html = '<span class="text-' + color + '"><i class="fa fa-circle"></i> ' + __(valueArr[value]) + '</span>';
                    return html;
                },
                ...
                datetime: function (value, row, index) {
                    return value ? Moment(value).format("YYYY-MM-DD HH:mm:ss") : __('None');
                },
