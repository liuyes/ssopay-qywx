    ...
    var Controller = {
        ...
        login: function () {
			...
			$('#captchaImg').click(function(){
				$('#captchaImg').attr("src","/code?"+Math.random());
			});
            ...
            $("#login-form").submit(function(){
            	//刷新验证码
            	//防止com.octo.captcha.service.CaptchaServiceException: Invalid ID, could not validate unexisting or already validated captcha
                $('#captchaImg').attr("src","/code?"+Math.random());
            });

            //为表单绑定事件
            Form.api.bindevent($("#login-form"), null, function (data) {
            	location.href = ctxUrl + "\/index";
            });
        }
    };
